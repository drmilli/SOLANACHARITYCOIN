...
export function Waves({
  ...
  const opacityRef = useRef(1) // Reference to track opacity
  ...
  useEffect(() => {
      ...
      lines.forEach((pts) => {
        pts.forEach((p) => {
          ...
          if (dist < l) {
            ...
            // Reduce the influence factor for subtler mouse interaction
    ...
    function drawLines() {
      ...
      const color = lineColor.startsWith('rgba') ? lineColor.replace(/[\d.]+\)$/, `${opacityRef.current})`) : lineColor

      // Make the lines thinner and smoother
    ...
    function tick(t) {
      ...
      // Skip animation calculations if fully transparent
      // but keep the animation loop running
