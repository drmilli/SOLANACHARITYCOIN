...
const RafflePageContent = () => {
  ...
  const [raffles, setRaffles] = useState<{ publicKey: PublicKey; account: RaffleState }[]>([])
  const [selectedRaffle, setSelectedRaffle] = useState<string | null>(null)
  const [donationAmount, setDonationAmount] = useState(0.01)
  const [loading, setLoading] = useState(false)
  const [statusMessage, setStatusMessage] = useState('')

  // Fetch raffle state accounts
  ...
  // Fetch raffle data when program is initialized
  useEffect(() => {
    if (program) {
      fetchRaffleState()
    }
  ...
  const donateToRaffle = async () => {
    if (!program || !wallet || !wallet.publicKey || !selectedRaffle || !currentRaffleState) {
      const missingPieces = []
      if (!program) missingPieces.push('program not initialized')
      if (!wallet || !wallet.publicKey) missingPieces.push('wallet not connected')
      if (!selectedRaffle || !currentRaffleState) missingPieces.push('no raffle selected')
  ...
  const handleRaffleSelect = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  // Get formatted raffle options
  const getRaffleOptions = useCallback(() => {
    return raffles.map((raffle, idx) => ({
      value: raffle.publicKey.toString(),
      label: `Raffle #${idx + 1}: ${raffle.publicKey.toString().substring(0, 8)}...`,
      totalSol: formatSOL(raffle.account.totalSol)
    }))
  }, [raffles])

  const currentRaffleState = getCurrentRaffleState()
  const raffleOptions = getRaffleOptions()
  ...
  const calculateWinProbability = () => {
    if (!currentRaffleState || !currentRaffleState.participants || currentRaffleState.participants.length === 0 || !wallet.publicKey)
      return '0%'

    // Check if user has participated
    const userEntries = currentRaffleState.participants?.find((p) => wallet.publicKey && p.wallet.equals(wallet.publicKey))

    if (userEntries) {
      const userTokens = userEntries.tokens.toNumber()
      let totalTokens = 0

      for (const participant of currentRaffleState.participants) {
                ...
                <div>
                  <div className="mb-6">
                    ...
                    >
                      ...
                      {raffleOptions.map((option) => (
                        <option key={option.value} value={option.value}>
                          {option.label} ({option.totalSol} SOL)
                        </option>
                      ))}
                    </select>
                  </div>
                    ...
                    <div className="space-y-6">
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span>
                            <span>Progress: {formatSOL(currentRaffleState?.totalSol)} / {THRESHOLD_SOL} SOL</span>
